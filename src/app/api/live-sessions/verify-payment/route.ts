...
export async function POST(req: Request) {
  try {
    ...
    // 1. Verify Payment Signature (if signature is provided)
    ...
    if (razorpay_order_id && razorpay_payment_id && razorpay_signature && razorpayKeySecret) {
      ...
      // If direct payment ID is present in test mode
      isSignatureValid = Boolean(razorpay_payment_id);
    }

    // 2. Update Supabase tbl_LiveSessionReg
    ...
    // 2026-09-21 14:00 to 15:00 IST is 20260921T083000Z to 20260921T093000Z (UTC)
    ...
    if (resendApiKey && email) {
      try {
        const candidateHtml = `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #0B0428; color: #FFFFFF; border: 1px solid #1E1742; border-radius: 20px; overflow: hidden; padding: 32px;">
            <div style="text-align: center; margin-bottom: 24px;">
              ...
              <p style="color: #9CA3C7; font-size: 14px; margin-top: 6px;">Thank you, <strong style="color: #FFFFFF;">${name}</strong>. We look forward to seeing you live.</p>
            </div>
        ...
        console.error('Failed to send session confirmation email via Resend:', e);
      }
    }
